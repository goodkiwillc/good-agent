...
class TestCitationManagerInstallation:
    ...
    async def test_manager_installation(self):
        ...
        async with Agent("Test assistant", extensions=[manager]) as agent:
...
class TestRealWorldScenarios:
    ...
    async def test_research_workflow_scenario(self):
        ...
        manager = CitationManager()
        async with Agent("Research Assistant", extensions=[manager]) as agent:
    ...
    async def test_perplexity_style_response_processing(self):
        ...
        async with Agent("Research Assistant", extensions=[manager]) as agent:
            ...
            assert (
                "[nature.com](https://nature.com/climate-study-2024)" in display_content
            )
    ...
    async def test_search_result_with_existing_citations(self):
        ...
        async with Agent("Research Assistant", extensions=[manager]) as agent:
            ...
            assert len(manager.index) == 2
            assert (
                manager.index.lookup(URL("https://existing-source-1.com/article")) == 1
            )
            assert (
                manager.index.lookup(URL("https://existing-source-2.com/research")) == 2
            )
...
class TestCitationManagerAPI:
    ...
    async def test_export_citations(self):
        ...
        manager = CitationManager()
        async with Agent("Test assistant", extensions=[manager]):
...
class TestCitationAdapterIntegration:
    ...
    async def test_complete_citation_adapter_workflow(self):
        ...
        async with Agent(
            ...
            assert "url" in transformed_params, (
                "url should be present after transformation"
            )
            ...
            assert (
                result.response == f"Content from {manager.index.get_url(research_idx)}"
            )

            # Step 6: Test parameter transformation - multiple URLs
            llm_multi_params = {
                "citation_idxs": [docs_idx, repo_idx],
                "parallel": True,
            }
            ...
            multi_result = await fetch_urls_tool(
                _agent=agent, **transformed_multi_params
            )
