...
class TestLLMIntegration:
    ...
    async def test_call_simple_response(self):
        ...
        agent_with_tools = await create_test_agent()
        # Mock the LLM response
        mock_response = MockLLMResponse(content="Hello! How can I help you today?")

        with patch.object(
            agent_with_tools.model, "complete", AsyncMock(return_value=mock_response)
        ):
            # Make the call
    ...
    async def test_messages_to_litellm_conversion(self):
        ...
        agent_with_tools.messages.append(
            ToolMessage(
                content="Result: 4", tool_call_id="call_456", tool_name="calculate"
            )
        )
...
class TestExecuteMethod:
    ...
    async def test_execute_with_tool_execution(self):
        ...
        responses = [
            MockLLMResponse(
                "I'll check the weather in London for you.", tool_calls=[mock_tool_call]
            ),
            MockLLMResponse(
        ...
        with patch.object(
            agent_with_tools.model, "complete", AsyncMock(side_effect=responses)
        ):
            # Execute and collect messages
            messages = []
                    ...
                    case AssistantMessage(i=current_iteration, content=content):
                        ...
                        if "TRIGGER" in str(content) and current_iteration < 4:
                            ...
                            agent_with_tools.append(
                                "Here's additional context you requested", role="user"
                            )
