...
class TestTypedEventParameters:
    ...
    async def test_state_change_params(self):
        ...
        # Note: Using async context manager calls initialize automatically
        events_captured = []

        # We need to register the handler BEFORE initialize() is called by the context manager
        # So we create the agent first, register handler, then enter context
...
class TestTypedEventHandlersMixin:
    ...
    async def test_mixin_basic_usage(self):
        ...
        async with TypedAgent("Test system") as agent:
            message_events = []
            tool_events = []
