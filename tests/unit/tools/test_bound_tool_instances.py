...
async def test_tool_methods_are_bound_tool_descriptors():
    ...
    assert isinstance(class_attr, BoundTool), (
        "Expected BoundTool descriptor at class level"
    )
...
async def test_bound_tools_work_with_agent():
    ...
    if (
        hasattr(component, "_tool_registration_task")
        and component._tool_registration_task
    ):
