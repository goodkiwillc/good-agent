"""
CONTEXT: Core message system for agent conversations with content parts and rendering.
ROLE: Defines message types, content management, and rendering pipeline for AI interactions.
DEPENDENCIES:
  - good_agent.models for base model infrastructure
  - .content for content part system (text, templates, images, files)
  - .interfaces for rendering protocols (LLM, display, string)
  - .tools for tool call integration
ARCHITECTURE:
  - Message hierarchy with role-specific subclasses
  - Content parts system for multimodal content
  - Template rendering with context resolution
  - Versioning support for message history management
  - Rendering pipeline with multiple output modes
KEY EXPORTS: Message classes, MessageList, FilteredMessageList, content parts
USAGE PATTERNS:
  1. Create messages: UserMessage("Hello"), AssistantMessage(response="Hi there!")
  2. Template messages: SystemMessage("You are a {role} assistant")
  3. Content parts: Message(text_part, image_part, template_part)
  4. Message lists: MessageList[Message] with versioning support
RELATED MODULES:
  - agent.py: Message consumption and execution
  - content.py: Content part implementations
  - interfaces.py: Rendering protocol definitions
  - tools.py: Tool call message integration

PERFORMANCE CONTEXT:
- Message creation: ~1-5ms depending on content complexity
- Template rendering: ~10-50ms for complex templates with context
- Memory usage: ~1KB per message + content size
- Thread safety: Thread-safe for read operations, versioning for concurrent access

RENDERING PIPELINE:
Messages support multiple rendering modes through content parts:
- LLM: Optimized for language model consumption (templates resolved)
- DISPLAY: Human-readable output with formatting
- RAW: Unprocessed content for debugging
- STORAGE: Serialized format for persistence

CONTENT PARTS SYSTEM:
- TextContentPart: Plain text with optional template detection
- TemplateContentPart: Jinja2 templates with context variables
- ImageContentPart: Image data with URL or bytes
- FileContentPart: Generic file attachments
- Parts are rendered independently then concatenated

TEMPLATE SYSTEM:
- Jinja2-based templating with sandboxing for security
- Context variables resolved from agent context chainmap
- Template caching for performance optimization
- Recursive rendering prevention with thread-local guards

VERSIONING INTEGRATION:
- Messages support versioning through MessageList integration
- Each message has unique ULID for tracking
- Version manager tracks message history and changes
- Forking and rollback operations supported
"""
