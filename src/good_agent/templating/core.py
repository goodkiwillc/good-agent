"""
CONTEXT: Core template rendering system with hierarchical context resolution and file-based template support.
ROLE: Provides template compilation, rendering, caching, and context management for agent message processing.
DEPENDENCIES:
  - good_agent.templating: Core Jinja2 environment and template registry
  - jinja2.ChoiceLoader: Template loading from multiple sources
  - .injection: Dependency injection for context providers
  - .environment: Template environment creation and security
  - .storage: File-based template loading and caching
ARCHITECTURE:
  - TemplateManager: Central coordinator for template operations
  - ContextResolver: Hierarchical context resolution with dependency injection
  - Template Registry: In-memory template storage and lookup
  - File Storage: Chained template loading from multiple directories
  - Template Cache: Performance optimization for repeated renders
KEY EXPORTS: TemplateManager, Template wrapper, global_context_provider decorator
USAGE PATTERNS:
  1. Template rendering: agent.template.render("Hello {{name}}", {"name": "User"})
  2. File templates: agent.template.render_template("{% include 'system/assistant' %}")
  3. Context providers: @agent.template.context_provider("current_time")
  4. Template wrapper: search_tool(query=Template("{{subject}} {{region}}"))
RELATED MODULES:
  - messages.py: Template content parts and message rendering
  - agent.py: Template manager integration and context access
  - context.py: Hierarchical context management
  - components.py: Agent component base class and lifecycle

PERFORMANCE CONTEXT:
- Template compilation: 5-20ms for complex templates with inheritance
- Template rendering: 1-10ms for typical templates (cached compilation)
- Context resolution: 1-5ms for provider chain resolution
- File template loading: 10-50ms initially, cached thereafter
- Memory usage: ~1KB per compiled template + context storage

RENDERING PIPELINE:
1. Template Discovery: Check file storage, then registry, then inline templates
2. Context Resolution: Build hierarchical context with provider injection
3. Template Compilation: Parse and compile Jinja2 templates (cached)
4. Rendering: Execute template with resolved context variables
5. Event Integration: Fire template lifecycle events for modifications
6. Caching: Cache compiled templates and rendered results when appropriate

CONTEXT RESOLUTION HIERARCHY:
1. Base Context: Direct context parameters passed to render methods
2. Context Stack: Temporary context overrides via context managers
3. Message Context: Context from specific message being rendered
4. Instance Providers: Agent-specific context providers
5. Global Providers: System-wide context providers (today, now, etc.)
6. Default Values: Fallback values for missing variables

FILE TEMPLATE LOADING:
Template discovery follows a priority order:
1. Explicit Directory: Specified prompts directory (highest priority)
...
- Template compilation errors are fatal for security

DEPENDENCY INJECTION:
Context providers support dependency injection:
- Agent instance injection for agent-specific providers
- Message injection for message-aware providers
- Context variable injection from resolved context
- Async provider support for dynamic data fetching
"""
