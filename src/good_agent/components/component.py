"""
CONTEXT: Agent component base classes and extension system architecture.
ROLE: Provides foundation for agent extensions with lifecycle management, tool binding, and event integration.
DEPENDENCIES:
  - good_agent.utilities.event_router: Event handling and router integration
  - .tool_adapter: Tool adaptation and registry management
  - ..events: Agent-specific event definitions
ARCHITECTURE:
  - AgentComponent base class for extension development
  - AgentComponentType metaclass for automatic tool discovery
  - Component lifecycle management with setup/install phases
  - Event-driven architecture for component communication
KEY EXPORTS: AgentComponent, AgentComponentType, Component lifecycle classes
USAGE PATTERNS:
  1. Component creation: class MyComponent(AgentComponent)
  2. Tool binding: @tool decorator on component methods
  3. Event handling: @component.on(AgentEvents.EVENT_NAME)(handler)
  4. Dependency injection: Component dependencies via __depends__
RELATED MODULES:
  - agent.py: Agent class that manages component lifecycle
  - tools.py: Tool system integration
  - events/: Event definitions and handling
  - base.py: Index protocols for component registration

PERFORMANCE CONTEXT:
- Component discovery: ~1-5ms per component (tool method scanning)
- Tool binding: ~1ms per tool (signature generation)
- Event setup: ~1ms per event handler registration
- Memory usage: ~1-5KB per component + tool instances

COMPONENT LIFECYCLE:
1. Class Definition: Tool methods automatically discovered by metaclass
2. Instantiation: Component instance created with dependencies
3. Setup Phase: Synchronous setup and event handler registration
4. Install Phase: Async initialization and resource allocation
5. Runtime: Event handling and tool execution
6. Cleanup: Resource cleanup and event handler removal

EXTENSION PATTERNS:
- Tool methods: Methods decorated with @tool become available to agent
- Event handlers: Methods decorated with @component.on() listen to agent events
- Dependency injection: Components can depend on other components
- Configuration: Components can access agent configuration
"""
