"""
Component Message Injection System.

This module provides enhanced capabilities for AgentComponents to inject
content into system prompts and user messages at runtime while respecting
the component enablement state and utilizing the templating system.

Key Features:
-------------
- Runtime injection of content into system prompts and user messages
- Full support for the Agent library's templating system
- Respects component enablement state (can be toggled on/off)
- Non-destructive message modification using content parts
- Support for multiple injection components working together

Architecture:
------------
The injection system works by hooking into the Agent's event system:
- System prompts: Modified via MESSAGE_SET_SYSTEM_AFTER event
- User messages: Modified via MESSAGE_RENDER_BEFORE event

Messages use Pydantic's frozen models, so system messages are replaced
with new instances containing the injected content parts. User message
parts are modified in-place during rendering, which is non-destructive.

Usage:
------
1. Extend MessageInjectorComponent and override the injection methods:
   ```python
   class MyInjector(MessageInjectorComponent):
       def get_system_prompt_prefix(self, agent):
           return [TextContentPart(text="[PREFIX]")]
   ```

2. Use SimpleMessageInjector for quick text/template injection:
   ```python
   injector = SimpleMessageInjector(system_prefix="Important: ", system_suffix=" [END]")
   ```

3. Add to agent as an extension:
   ```python
   agent = Agent("System prompt", extensions=[injector])
   ```
"""
