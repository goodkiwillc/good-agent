"""
CONTEXT: Tool system for agent function binding and execution with dependency injection.
ROLE: Provides tool discovery, execution, and management for AI agent capabilities.
DEPENDENCIES:
  - fast_depends for dependency injection and parameter binding
  - pydantic for schema generation and validation
  - tenacity for retry logic and error handling
  - good_agent.models for base infrastructure
  - ..components for agent component integration
ARCHITECTURE:
  - Tool wrapping of functions with schema generation
  - ToolManager for discovery and lifecycle management
  - Dependency injection system for agent, context, and component access
  - Retry logic and error isolation for robust execution
  - MCP (Model Context Protocol) server integration
KEY EXPORTS: Tool class, ToolManager, tool decorator, ToolCall/ToolResponse models
USAGE PATTERNS:
  1. Function decoration: @tool async def my_func(param: str) -> str
  2. Manual wrapping: Tool(my_function, name="custom_name")
  3. Manager registration: agent.tools.register_tool(my_tool)
  4. Pattern-based loading: agent.tools.load_tools_from_patterns(["search:*"])
RELATED MODULES:
  - agent.py: Tool execution and integration
  - bound_tools.py: Method-based tool descriptors
  - registry.py: Global tool registration and discovery
  - mcp/: External tool server integration

PERFORMANCE CONTEXT:
- Tool creation: ~1-5ms for function wrapping and schema generation
- Tool execution: Function call time + dependency injection overhead (~1ms)
- Schema generation: ~5-20ms for complex functions with annotations
- Memory usage: ~1-5KB per tool + schema cache
- Concurrency: Tools execute independently, can run in parallel

TOOL EXECUTION FLOW:
1. Discovery: Tools registered locally or loaded from global registry/MCP servers
2. Schema Generation: Pydantic models created from function signatures
3. Dependency Injection: Agent, context, and components injected automatically
4. Execution: Function called with resolved parameters and error handling
5. Response: Results wrapped in ToolResponse with metadata

DEPENDENCY INJECTION SYSTEM:
- Agent instance: Automatically injected for agent access
...
- AgentComponent subclasses: Type-based component injection
- ContextValue descriptors: Template variable access

ERROR HANDLING STRATEGY:
- Tool failures: Captured in ToolResponse, execution continues
  - Retry logic with exponential backoff (if enabled)
  - Error context preserved for LLM understanding
  - No single tool failure stops agent execution
- System failures: Logged but isolated from agent flow
- Validation errors: Immediate failure before execution
- Dependency injection failures: Clear error messages

MCP INTEGRATION:
- External tool servers via Model Context Protocol
  - Automatic discovery and tool loading
  - Connection management and lifecycle
  - Tool schema translation and adaptation
  - Error handling and reconnection

SCHEMA GENERATION:
- Pydantic models created from function signatures
- Type annotations used for parameter validation
- Docstrings and comments used for descriptions
- Hidden parameters excluded from public schemas
- Custom JSON schema generation for LLM compatibility

TOOL CATEGORIES:
1. Direct Functions: Simple Python functions wrapped as tools
2. Component Methods: AgentComponent methods with dependency injection
3. MCP Tools: External server tools with protocol translation
4. Registry Tools: Globally registered tools with metadata
"""
