"""
CONTEXT: Mock and testing infrastructure for agent development and unit testing.
ROLE: Provides comprehensive mocking capabilities for agent components, LLM interactions, and tool execution.
DEPENDENCIES:
  - unittest.mock: Core mocking infrastructure
  - pydantic: Mock model validation and serialization
  - loguru: Structured logging for mock debugging
  - orjson: JSON serialization for mock data
ARCHITECTURE:
  - AgentMockInterface: Central mock coordination
  - MockLanguageModel: LLM response mocking
  - MockToolCall and MockResponse: Tool execution mocking
  - MockMessage: Message creation and validation
  - MockAgentConfigManager: Configuration mocking
KEY EXPORTS: Comprehensive mock classes for all agent components
USAGE PATTERNS:
  1. Unit testing: MockAgent(model=mock_model, tools=mock_tools)
  2. Development testing: MockLanguageModel().set_response("test response")
  3. Tool testing: MockToolCall(name="test_tool").set_response("result")
  4. Integration testing: MockQueuedLanguageModel for complex scenarios
RELATED MODULES:
  - agent.py: Main agent class being mocked
  - tools.py: Tool system being mocked
  - model/llm.py: Language model being mocked
  - messages.py: Message types being mocked

PERFORMANCE CONTEXT:
- Mock creation: ~1-5ms per mock instance
- Response simulation: <1ms for canned responses
- Memory usage: ~1-5KB per mock instance
- Thread safety: Thread-safe for read operations

MOCKING STRATEGY:
1. Realistic behavior simulation
2. Configurable response patterns
3. State tracking for debugging
4. Integration with real agent components
5. Support for both unit and integration testing

USAGE PATTERNS:
- Simple mocking: MockAgent().set_response("Hello")
- Complex scenarios: MockQueuedLanguageModel([response1, response2])
- Tool mocking: MockToolCall().set_response("tool result")
- Configuration mocking: MockAgentConfigManager(model="gpt-4")

TESTING INTEGRATION:
- pytest fixtures: conftest.py with mock instances
- VCR support: Recorded interactions for reproducible tests
- Performance testing: Controlled timing and latency simulation
