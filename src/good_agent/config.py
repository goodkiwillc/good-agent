"""
CONTEXT: Agent configuration management system with hierarchical context and typed fields.
ROLE: Provides configuration management with type safety, inheritance, and context manager support.
DEPENDENCIES:
  - collections.ChainMap: Hierarchical configuration inheritance
  - typing: Type hints and generic type validation
ARCHITECTURE:
  - ConfigField descriptor for typed configuration fields
  - ConfigStack for hierarchical configuration management
  - AgentConfigManager for agent-specific configuration
  - Type validation and inheritance mechanisms
KEY EXPORTS: ConfigField, ConfigStack, AgentConfigManager, TypedDict definitions
USAGE PATTERNS:
  1. Configuration inheritance: ConfigStack(base_config, local_config)
  2. Type-safe fields: ConfigField(default="value", type_hint=str)
  3. Context managers: Temporary configuration overrides
  4. Agent configuration: AgentConfigManager(model="gpt-4", temperature=0.7)
RELATED MODULES:
  - agent.py: Uses configuration for model and behavior settings
  - context.py: Inherits from ConfigStack for context management
  - tools.py: Tool configuration and registration patterns

PERFORMANCE CONTEXT:
- Configuration lookup: O(1) for ChainMap-based inheritance
- Type validation: ~1ms per field assignment (minimal overhead)
- Memory usage: ~1KB base + configuration storage
- Thread safety: Thread-safe for read operations

CONFIGURATION INHERITANCE:
1. Local configuration (highest priority)
2. Base configuration (fallback values)
3. Default field values (lowest priority)
4. Type validation at each level

USAGE PATTERNS:
- Model configuration: model="gpt-4", temperature=0.7
- Tool configuration: tools=[tool1, tool2], tool_timeout=30
- Template configuration: template_path="./templates"
- Validation configuration: message_validation_mode="strict"
"""
