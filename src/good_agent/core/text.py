"""
CONTEXT: Comprehensive text processing utilities for content normalization and formatting.
ROLE: Provide text normalization, wrapping, bullet processing, Unicode handling, and formatting
      utilities for consistent content processing across goodintel_core.
DEPENDENCIES: numpy for array operations, standard library for text processing and Unicode handling.
ARCHITECTURE: Modular text processing utilities organized by function: normalization, wrapping,
              bullet processing, encoding/decoding, and formatting.
KEY EXPORTS: string (TextProcessor instance with various text methods)
USAGE PATTERNS:
  1) Use string.normalize() for text normalization and cleanup
  2) Apply string.wrap() for consistent text formatting with width control
  3) Process bullet points and lists with string.extract_bullets()
  4) Handle Unicode and encoding issues with utility methods
RELATED MODULES:
  - goodintel_core.models.entities: Entity model text processing
  - goodintel_core.models.renderable: Template text formatting
  - goodintel_core.templating: Template text processing
"""
