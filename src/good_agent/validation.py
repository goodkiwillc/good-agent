"""
Message sequence validation for ensuring LLM-compatible message ordering.

CONTEXT: Message sequence validation system for LLM compatibility and conversation integrity.
ROLE: Ensures message sequences follow proper patterns for language model consumption and tool execution.
DEPENDENCIES:
  - .messages: Message types and validation patterns
  - loguru: Structured logging for validation warnings
ARCHITECTURE:
  - ValidationMode enum for different validation strictness levels
  - MessageSequenceValidator for comprehensive validation logic
  - ValidationError exception for strict mode failures
  - Tool sequencing validation for tool call/response patterns
KEY EXPORTS: ValidationMode, MessageSequenceValidator, ValidationError
USAGE PATTERNS:
  1. Agent initialization: MessageSequenceValidator(mode=ValidationMode.STRICT)
  2. Message validation: validator.validate(message_list)
  3. Partial validation: validator.validate_partial_sequence(messages, allow_pending_tools=True)
RELATED MODULES:
  - agent.py: Uses validator during message processing
  - messages.py: Message types being validated
  - tools.py: Tool call/response patterns being validated

PERFORMANCE CONTEXT:
- Validation overhead: ~1-5ms for typical conversations (<50 messages)
...
VALIDATION RULES:
1. Tool Call Sequencing: Tool responses must immediately follow tool calls
2. Role Alternation: Proper user/assistant message patterns
3. System Message Placement: System messages at beginning or specific positions
4. Tool Call Completeness: All tool calls must have corresponding responses
5. Message Type Compatibility: Valid role transitions and message ordering

ERROR HANDLING:
- STRICT: Raises ValidationError for any validation issues
- WARN: Logs warnings but allows execution to continue
- SILENT: No validation performed (for performance or legacy compatibility)
"""
