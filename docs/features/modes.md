...
async with Agent("You are a versatile assistant.") as agent:
    # @TODO: look at the original DESIGN.md - we were passing the agent: Agent instance - not ModeContext - why was this changed? does ModeContext have agent as a property?
...
async def session_mode(ctx: ModeContext):
    # @TODO: need to verify we have a test that confirms that agent is initialized once a mode is entered.
